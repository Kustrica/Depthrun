...
public:
	...
	/** K parameter for sigmoid distance transform. Higher = sharper mid-range spike. */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Adaptive|Transforms")
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Adaptive|ThreatFormula")
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Adaptive|Thresholds")
	int32 AdaptiveThresholdWindow = 20;

	// ─── Transition Cost Matrix [From][To] (5x5 = None,Idle,Chase,Attack,Retreat,Flank) ──
	// Stored flat [row*5 + col], indexed by EFSMStateType cast to int (skip None=0)
	// Init in DA asset or via code; default values match the plan's 5x5 table.

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Adaptive|CostMatrix")
	TArray<float> TransitionCostMatrix;

	// ─── Utility Curve Parameters (per state) ────────────────────────────────

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Adaptive|UtilityCurves")
